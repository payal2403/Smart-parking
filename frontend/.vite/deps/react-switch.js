...
//#endregion
//#region node_modules/react-switch/dist/index.dev.mjs
...
var uncheckedIcon = import_react.createElement("svg", {
	viewBox: "-2 -5 14 20",
	height: "100%",
	width: "100%",
	style: {
...
var checkedIcon = import_react.createElement("svg", {
	height: "100%",
	width: "100%",
	viewBox: "-2 -5 17 21",
	style: {
...
function createBackgroundColor(pos, checkedPos, uncheckedPos, offColor, onColor) {
	...
	if (relativePos === 0) return offColor;
	if (relativePos === 1) return onColor;
	var newColor = "#";
	for (var i = 1; i < 6; i += 2) {
		var offComponent = parseInt(offColor.substr(i, 2), 16);
		var onComponent = parseInt(onColor.substr(i, 2), 16);
		var newComponent = Math.round((1 - relativePos) * offComponent + relativePos * onComponent).toString(16);
		if (newComponent.length === 1) newComponent = "0" + newComponent;
...
function convertShorthandColor(color) {
	...
	var sixDigitColor = "#";
	for (var i = 1; i < 4; i += 1) sixDigitColor += color[i] + color[i];
	return sixDigitColor;
}
function getBackgroundColor(pos, checkedPos, uncheckedPos, offColor, onColor) {
	return createBackgroundColor(pos, checkedPos, uncheckedPos, convertShorthandColor(offColor), convertShorthandColor(onColor));
}
var hexColorPropType = function(props, propName, componentName) {
	var prop = props[propName];
	if (typeof prop !== "string" || prop[0] !== "#" || prop.length !== 4 && prop.length !== 7) return /* @__PURE__ */ new Error("Invalid prop '" + propName + "' supplied to '" + componentName + "'. '" + propName + "' has to be either a 3-digit or 6-digit hex-color string. Valid examples: '#abc', '#123456'");
	return null;
};
...
var ReactSwitch = /* @__PURE__ */ function(Component) {
	function ReactSwitch(props) {
		...
		this.state = { $pos: checked ? this.$checkedPos : this.$uncheckedPos };
		this.$lastDragAt = 0;
		this.$lastKeyUpAt = 0;
	...
	ReactSwitch.prototype.$onDrag = function $onDrag(clientX) {
		...
		var $pos = ref.$pos;
		var mousePos = (this.props.checked ? this.$checkedPos : this.$uncheckedPos) + clientX - $startX;
		if (!$isDragging && clientX !== $startX) this.setState({ $isDragging: true });
		var newPos = Math.min(this.$checkedPos, Math.max(this.$uncheckedPos, mousePos));
		if (newPos !== $pos) this.setState({ $pos: newPos });
	};
	ReactSwitch.prototype.$onDragStop = function $onDragStop(event) {
		...
		if (!$isDragging || timeSinceStart < 250 || checked && $pos <= halfwayCheckpoint || !checked && $pos >= halfwayCheckpoint) this.$onChange(event);
	...
	ReactSwitch.prototype.$onInputChange = function $onInputChange(event) {
		if (Date.now() - this.$lastDragAt > 50) {
			...
			if (Date.now() - this.$lastKeyUpAt > 50) {
				if (this.$isMounted) this.setState({ $hasOutline: false });
			}
		}
	};
	...
	ReactSwitch.prototype.render = function render() {
		...
		var uncheckedHandleIconStyle = {
			height: this.$handleDiameter,
			width: this.$handleDiameter,
			opacity: Math.max((1 - ($pos - this.$uncheckedPos) / (this.$checkedPos - this.$uncheckedPos) - .5) * 2, 0),
			position: "absolute",
			left: 0,
			top: 0,
			pointerEvents: "none",
		...
		var checkedHandleIconStyle = {
			height: this.$handleDiameter,
			width: this.$handleDiameter,
			opacity: Math.max((($pos - this.$uncheckedPos) / (this.$checkedPos - this.$uncheckedPos) - .5) * 2, 0),
			position: "absolute",
			left: 0,
			top: 0,
			pointerEvents: "none",
		...
		}, checkedIcon && import_react.createElement("div", { style: checkedIconStyle }, checkedIcon), uncheckedIcon && import_react.createElement("div", { style: uncheckedIconStyle }, uncheckedIcon)), import_react.createElement("div", {
		...
		}, uncheckedHandleIcon && import_react.createElement("div", { style: uncheckedHandleIconStyle }, uncheckedHandleIcon), checkedHandleIcon && import_react.createElement("div", { style: checkedHandleIconStyle }, checkedHandleIcon)), import_react.createElement("input", _extends({}, {
			type: "checkbox",
			role: "switch",
...
ReactSwitch.propTypes = {
	...
	uncheckedIcon: import_prop_types.default.oneOfType([import_prop_types.default.bool, import_prop_types.default.element]),
	checkedIcon: import_prop_types.default.oneOfType([import_prop_types.default.bool, import_prop_types.default.element]),
